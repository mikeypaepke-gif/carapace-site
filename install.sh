...
carapace_restart_gateway_and_wait_ready() {
  ...
  start=$(date +%s)
  while :; do
    if timeout 10 openclaw cron list >/dev/null 2>&1; then
      now=$(date +%s)
...
# embedded acpx runtime can lazy-load after process start. Step 9's
# wait_ready does the real readiness probe.
...
# ══════════════════════════════════════════════════════════
# Carapace is a shell on top of OpenClaw. Provider/model/gateway are
# already configured by OpenClaw. This step layers in the Carapace
# workspace rules, caps, clean session state, and readiness checks.
...
# Detect OpenClaw workspaces and inject Carapace rules. If only main is
# detected, no prompt fires. Non-interactive runs default to all.
carapace_select_workspaces
...
# 2. Tailscale serve routes registered.
...
# 3. Workspace prompts injected. Accept either the new CARAPACE-STARTUP
# or a managed AGENTS block plus standard workspace files.
WS_COUNT=$(ls "$HOME/.openclaw/workspace/"*.md 2>/dev/null | wc -l || echo 0)
if (( WS_COUNT >= 5 )) && grep -q "BEGIN CARAPACE" "$HOME/.openclaw/workspace/AGENTS.md" 2>/dev/null; then
  VERIFY_WORKSPACE=true
  ok "Workspace prompts present ($WS_COUNT files)"
else
  warn "Workspace prompts incomplete (found $WS_COUNT .md files; check ~/.openclaw/workspace/AGENTS.md)"
fi

# 4. Gateway token.
TOKEN=$(python3 -c 'import json,os; print(json.load(open(os.path.expanduser("~/.openclaw/openclaw.json")))["gateway"]["auth"]["token"])' 2>/dev/null || echo "")
if [ -z "$TOKEN" ]; then
  timeout 30 openclaw gateway install >> "$LOGFILE" 2>&1 || true
  TOKEN=$(python3 -c 'import json,os; print(json.load(open(os.path.expanduser("~/.openclaw/openclaw.json")))["gateway"]["auth"]["token"])' 2>/dev/null || echo "")
fi
...
# Final cleanup: silence nvm-vs-npm-prefix warning forever.
if [[ -f "$HOME/.npmrc" ]]; then
  sed -i '/^prefix=/d;/^globalconfig=/d' "$HOME/.npmrc" 2>/dev/null || true
  [[ -s "$HOME/.npmrc" ]] || rm -f "$HOME/.npmrc"
fi

# Pre-heat the agent runtime + provider with one real chat completion so
# the user's first iOS message after pairing isn't stuck in cold-start.
# Always returns 0.
...
echo ""
echo -e "  ${BOLD}Helpers:${RESET} ${BOLD}carapace-qr${RESET} (re-show QR) · ${BOLD}carapace-onboard${RESET} (re-run AI setup) · ${BOLD}carapace-prune${RESET} (safe session cleanup)"
...
if [ "${CODEX_OAUTH_NEEDED:-false}" = "true" ]; then
  ...
  echo ""
  echo -e "  ${DIM}Open the URL on any device with a browser, sign in, approve, then scan the QR above.${RESET}"
  echo ""
fi
